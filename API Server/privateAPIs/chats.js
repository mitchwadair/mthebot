...
const get = (req, res) => {
    ...
    DBService.getChannel(channel)
        .then((data) => {
            if (data) res.status(200).send(data.enabled.toString());
            else res.status(404).send(`Channel ${encodeURIComponent(channel)} not found`);
        })
        .catch((err) => {
...
const post = (actions, req, res) => {
    const channel = req.params.channel;
    DBService.enableChannel(channel)
        .then(() => {
            actions
                .joinChannel(channel)
                .then(() => {
...
const remove = (actions, req, res) => {
    const channel = req.params.channel;
    DBService.disableChannel(channel)
        .then(() => {
            actions
