...
MtheBot_ is built using [tmi.js](https://tmijs.com/) as well as [TESjs](https://tesjs.net/).  The back-end is built using a custom API server which hosts both a private API used by the front-end as well as a public API for general data (used in the "users" tag on this repo for example).  The bot's back end is run through [AWS](https://aws.amazon.com) so it has little-to-no downtime.
